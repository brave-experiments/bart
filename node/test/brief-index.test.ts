...
test('finding/check definitions, metadata and descriptive links agree', async (t) => {
  ...
  const cases = [
    ...
    { name: 'extra fragment is not a valid destination', index, finding,
      plan: plan.replace('#finding-001', '#finding-001#typo'), error: /Invalid finding\/check link/ },
    ...['`', '``'].map(marker => ({
      name: `${marker.length}-backtick code span cannot supply links`, index, finding,
      plan: '<a id="check-001"></a>\n' + marker + plan.split('\n').slice(1).join('\n') + marker,
      error: /must link to/,
    })),
    { name: 'inline code cannot supply an anchor', index, finding: '`' + finding + '`', plan, error: /Explicit anchors/ },
    { name: 'unmatched backtick does not hide real navigation', index, finding, plan: '` unmatched\n' + plan, error: undefined },
    { name: 'different delimiter inside code does not end the span', index, finding,
      plan: '<a id="check-001"></a>\n``literal ` ' + plan.split('\n').slice(1).join('\n') + '``', error: /must link to/ },
